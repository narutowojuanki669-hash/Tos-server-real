# main.py — Town of Shadows (Step 4 upgrade)
# Full backend including private night messages & day phases.
...
app = FastAPI(title="Town of Shadows - Step 4 (Private Night Actions)")
...
async def send_to_player(room_id: str, player_name: str, message: dict):
    """Send a private message to a player (if connected)."""
...
async def send_to_faction(room_id: str, faction: str, message: dict):
    """Send a message to all connected members of a faction (private faction chat)."""
    room = rooms.get(room_id)
    if not room: return
    for p in room["players"]:
...
async def handle_ws_message(room_id: str, wsid: str, msg: dict):
    ...
    if mtype == "identify":
        ...
        if p:
            p["ws_id"] = wsid
            p["is_bot"] = False
            # send private role to the player
            try:
        ...
        if ch == "mafia":
            ...
            # send chat to dead/medium (dead players)
            for p in room["players"]:
...
async def simulate_bot_night_actions(room_id: str):
    ...
    if mafia:
        ...
        if candidates:
            ...
            await send_to_faction(room_id, "Mafia", {"type":"system","text": f"🤖 Mafia targeted {victim['name']}"})
    # Cult convert attempt (private to cult)
    ...
    if cults and random.random() < 0.45:
        ...
        if candidates:
            ...
            await send_to_faction(room_id, "Cult", {"type":"system","text": f"🤖 Cult attempted to convert {t['name']}"})
    # Doctor bots heal (private to doctor and healed)
    ...
    # Bodyguard bots protect (private confirmations)
    ...
    # Vigilante bots (shoot mafia only) — private confirm to vigilante
    ...
    for j in [p for p in alive if p["role"]=="Jailor"]:
        tgt_obj = random.choice([p for p in alive if p["name"]!=j["name"]])
        tgt = tgt_obj["name"]
...
# ---------------- NIGHT RESOLUTION (private confirmations only) ----------------
async def apply_player_actions(room_id: str):
    ...
    for act in actions:
        ...
        if a_type == "doctor_heal":
            ...
            # private confirmations already sent during scheduling
    ...
    for conv in queued_converts:
        ...
        if target_p:
            if target_p["role"] not in ("Godfather","Mafioso","Beastman","Soldier"):
                ...
                converted_names.append(tname)
                # Notify converted player privately
                await send_to_player(room_id, tname, {"type":"system","text": "You were contacted by the Cult and are now their Acolyte."})
                # Notify cult privately that conversion succeeded (leader/fanatic/known cult)
                await send_to_faction(room_id, "Cult", {"type":"system","text": f"{tname} has joined the Cult."})
    ...
    for k in queued_kills:
        ...
        if lover:
            ...
            if lover_p:
                ...
                # private notify lover and actor
    ...
    for ent in final_kills:
        ...
        if name in protected_by_bodyguard:
            ...
            if bg_p:
                if actor_role == "Beastman" or ktype in ("beast_kill","beastman_kill"):
                    ...
                    await send_to_player(room_id, name, {"type":"system","text": f"Someone died protecting you tonight."})
                else:
                    bg_p["alive"] = False
                    deaths.append(bg_p["name"])
        ...
        if target_p and target_p.get("role") == "Soldier" and not target_p.get("soldier_used", False):
            if actor_role == "Beastman" or ktype in ("beast_kill","beastman_kill"):
                ...
                deaths.append(name)
                await send_to_player(room_id, name, {"type":"system","text": f"You were killed by Beastman."})
                continue
            else:
                target_p["soldier_used"] = True
                await send_to_player(room_id, name, {"type":"system","text": f"You used your one-time protection and survived."})
                continue

        # doctor protection
        bypass = (actor_role == "Beastman") or (ktype in ("beast_kill","beastman_kill"))
        if (name in protected_by_doctor) and (not bypass):
            await send_to_player(room_id, name, {"type":"system","text": f"You were attacked but survived due to Doctor protection."})
            continue
    ...
    # janitor cleans (private to janitor)
    for cleaned in janitor_cleans:
        room.setdefault("cleaned_bodies", set()).add(cleaned)
        # find janitor who requested this (we just notify janitors privately that cleaning happened)
        for p in room["players"]:
